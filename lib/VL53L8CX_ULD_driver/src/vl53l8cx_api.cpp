	...
	VL53L8CX_Configuration *p_dev)
{
	...
	status |= VL53L8CX_WrMulti(&(p_dev->platform), VL53L8CX_UI_CMD_END - (uint16_t)(4 - 1), (uint8_t *)cmd, sizeof(cmd));
	status |= _vl53l8cx_poll_for_answer(p_dev, 4, 1,
										VL53L8CX_UI_CMD_STATUS, 0xff, 0x03);
	...
	{
		Serial.println(tmp);
		Serial.println(p_dev->data_read_size);
		status |= VL53L8CX_STATUS_ERROR;
	}
		...
		if (p_results->nb_target_detected[i] == (uint8_t)0)
		{
				 ...
				 j++)
			{
				...
				p_results->target_status
					[((uint32_t)VL53L8CX_NB_TARGET_PER_ZONE * (uint32_t)i) + j] = (uint8_t)255;
#endif
			}
		}
	}
	...
	uint32_t *p_repeat_count)
{
	...
	*p_repeat_count = ((uint32_t)p_dev->temp_buffer[7] << 24) | ((uint32_t)p_dev->temp_buffer[6] << 16) | ((uint32_t)p_dev->temp_buffer[5] << 8) | (uint32_t)p_dev->temp_buffer[4];
